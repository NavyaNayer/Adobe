...
def main():
    ...
    # Generalize: extract keywords from persona/job and document titles
    persona_keywords = set(str(persona.get('role', '')).lower().split())
    job_keywords = set(str(job.get('task', '')).lower().split())
    doc_keywords = set()
    for doc in documents:
        doc_keywords.update(re.findall(r'\w+', doc.get('title', '').lower()))
    all_keywords = persona_keywords | job_keywords | doc_keywords
    for doc in documents:
        ...
        # Generalized scoring: boost for persona/job/doc keywords, penalize verbose or generic
        candidate_sections = []
        for h in outline:
            ...
            overlap = len(title_words & all_keywords)
            bonus += overlap * 5
            # Penalize verbose headings
            if word_count > 12 or char_count > 80:
                bonus -= 5
            # Penalize generic headings
            if title.lower() in {"conclusion", "introduction", "summary", "abstract", "table of contents"}:
                bonus -= 10
            if re.match(r"^chapter|^section", title.lower()):
                bonus -= 6
            # Penalize if title looks like a sentence
